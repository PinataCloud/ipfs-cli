...
OPTIONS:
   ...
   --setup-token  Store an Anthropic setup token instead of an API key (anthropic only)
   --help, -h     show help
```
...
|----------|-------------|---------|
| `anthropic` | `ANTHROPIC_API_KEY` | `--setup-token` for Claude Code setup tokens |
...
# Store Anthropic setup token (for Claude Code)
pinata agents auth anthropic --setup-token
